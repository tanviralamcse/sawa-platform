...
class ApplicationTest(TestCase):
    def test_create_application(self):
        user = User.objects.create(username='buyer2', role='buyer')
        ...
        user2 = User.objects.create(username='provider2', role='provider')
        provider = ProviderProfile.objects.create(user=user2, base_location='Berlin', education='BSc', years_experience=5)
        app = Application.objects.create(request=req, provider=provider, pitch='Choose me', status='submitted')
        self.assertEqual(app.pitch, 'Choose me')
